...
#### UI 增强
- [ ] 行号显示（基于 pretext lineCount，非 DOM 计数）
- [ ] 当前行高亮（光标所在行背景色）
- [ ] 编辑器 / 预览面板可拖拽调整宽度
- [ ] 全屏编辑 / 全屏预览切换
- [ ] 响应式布局（移动端上下排列）
...
#### 编辑增强
- [ ] Tab / Shift+Tab 缩进/反缩进（多行支持）
- [ ] 快捷键：Ctrl+B 加粗、Ctrl+I 斜体、Ctrl+K 链接、Ctrl+` 代码
- [ ] 自动补全：`- ` 列表续行、`> ` 引用续行、```` ``` ```` 代码块闭合
- [ ] 括号/引号自动配对（`**`、`__`、`` ` ``、`[]`、`()`）
- [ ] 选中文字后输入 `*` / `**` / `` ` `` 自动包裹
- [ ] 拖拽/粘贴图片自动插入 `![](url)` 占位

#### 工具栏
- [ ] Markdown 工具栏（标题、粗体、斜体、链接、图片、代码、列表、表格）
- [ ] 工具栏按钮操作插入对应语法到光标位置
